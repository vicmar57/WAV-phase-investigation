...
for data_block1, data_block2 in zip(data1,data2):
    ...
    #extract amp and phase (euler representaion of complex number)
    ...
    randPhase1 = np.random.uniform(0.0,2*np.pi,int(seg_size/2)+1)
    randPhase1[0]=0; randPhase1[-1]=0;
    randPhase2 = np.random.uniform(0.0,2*np.pi,int(seg_size/2)+1)
    randPhase2[0]=0; randPhase2[-1]=0;

    #switch phases
    switched1 = cartes(r1,randPhase1) #part1 amplitude, with phase of part2  np.linspace(0,math.pi*2.0,52001)
    switched2 = cartes(r2,randPhase2) #part2 amplitude, with phase of part1
...
wavfile.write("part1afterloopUniPhase.wav",samplerate, finalIFFT1)
wavfile.write("part2afterloopUniPhase.wav",samplerate, finalIFFT2)
